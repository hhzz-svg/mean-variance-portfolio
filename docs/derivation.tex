...
为排除"结论只是合成数据的特例"之疑，将同一套 walk-forward 协议与同一组策略原封不动
搬到 $8$ 只真实美股行业 ETF（$2012$--$2023$，复权日收益）上。结论以约 $89\%$ 的复制率
成立：等权 $1/N$ 仍是最难战胜的基准（真实样本外夏普 $0.62$）；切点（样本 $\muv$）在真实
市场失控更甚（换手 $3370\%$、最大回撤 $-3206\%$）；修 $\Sig$ 的各 GMV 变体仍簇拥在 $1/N$
之下；唯一明显胜出的是使用未来信息的"事后 $\muv$"（$0.98$），且其相对 $1/N$ 的优势在
block bootstrap 下仍不显著（$p=0.18$）。这表明前述结论源于均值--方差问题的结构本身，
而非合成数据生成过程的人为产物。
...
每种估计量的适用边界，解析非线性收缩则为协方差估计给出理论最优解，真实 ETF 数据上的
验证（复制率约 $89\%$）则确认这些结论源于问题结构本身。理论推导、代码实现、
